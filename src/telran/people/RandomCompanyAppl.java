...
public class RandomCompanyAppl {
	...
	private static List<Employee> getRandomEmployees() {
		...
		return Stream.generate(RandomCompanyAppl::getRandomEmployee).limit(nEmployees).toList();
	}

	private static Employee getRandomEmployee() {
		id++;
		return new Employee(id, "name" + id, getRandomDepartment(), getRandomNumber(minSalary, maxSalary));
	}
	...
	private static void fillConfigurationProperties() throws Exception {
		...
		try (InputStream input = new FileInputStream(PROPERTIES_FILE)) {
			...
			throw new Exception(String.format("properties file %s doesn't exist", PROPERTIES_FILE));
		}
