...
public class CopyFilesInputOutputStreams {
	...
	private static void copyFiles() throws Exception {
		...
		try (InputStream input = new FileInputStream(files[0]); OutputStream output = new FileOutputStream(files[1]);) {
			copyStreams(input, output);
		} catch (FileNotFoundException e) {
			throw new Exception(String.format(NO_DIRECTORY_DESTINATION, files[1].getName()));
	...
	private static void copyStreams(InputStream input, OutputStream output) throws Exception {
		...
		System.out.printf(SUCCESS_MESSAGE, files[0].getCanonicalPath(), files[1].getCanonicalPath(), totalLength,
